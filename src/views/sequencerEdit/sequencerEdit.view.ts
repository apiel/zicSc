...
import { RenderOptions, getView, viewPadPressed } from '../../view';
...
function patternController() {
    if (midiOutController !== undefined) {
        ...
        if (trackId !== undefined) {
            const { padColor } = getTrack(trackId);
...
eventSequencer.on('beatQuarter', async () => {
    if (getView() === View.SequencerEdit) {
        await sequencerEditView({ beatRendering: true });
        render();
    }
});
